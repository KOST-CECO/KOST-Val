		...
		<html>
			<head>
				<style>
					...
					tr.caption {background-color: #b2b2c5;
					font-weight:bold}
					td.captionw {background-color: #ffffff}
					tr.captionm {background-color: #f8dfdf}
					tr.captionio
					{background-color: #afeeaf; font-weight:bold}
					tr.captioniom
					{background-color: #ccffcc}
					tr.captioninfo {background-color:
					#e2e4e2}
					td.captioninfor {background-color: #e2e4e2; text-align:
					end}
				</style>
			</head>
			<body>
				...
				<xsl:for-each select="KOSTVal_FFCounter">
					<div>
						<table width="94%">
							<tr class="caption">
								<td>Zähler der verschiedenen Dateiformate im content der
									validierten SIPs</td>
							</tr>
						</table>
						<table width="94%">
							<tr class="captioninfo">
								<td width="12%" class="captioninfor">
									<xsl:value-of select="pdf" />
								</td>
								<td width="10%">PDF</td>
								<td width="2%" class="captionw">
								</td>
								<td width="12%" class="captioninfor">
							...
							<tr class="captioninfo">
								...
								<td width="12%" class="captioninfor">
									<xsl:value-of select="jp2" />
								</td>
								<td width="10%">JP2</td>
								<td width="2%" class="captionw">
								</td>
								<td width="12%" class="captioninfor">
									<xsl:value-of select="jpx" />
								</td>
								<td width="10%">JPX</td>
								<td width="2%" class="captionw">
								</td>
							...
							<tr class="captioninfo">
								...
								<td width="12%" class="captioninfor">
									<xsl:value-of select="xls" />
								</td>
								<td width="10%">XLS</td>
								<td width="2%" class="captionw">
								</td>
								<td width="12%" class="captioninfor">
									<xsl:value-of select="odt" />
								</td>
								<td width="10%">ODT</td>
								<td width="2%" class="captionw">
								</td>
								<td width="12%" class="captioninfor">
									<xsl:value-of select="ods" />
								</td>
								<td width="10%">ODS</td>
							</tr>
							<tr class="captioninfo">
								<td width="12%" class="captioninfor">
									<xsl:value-of select="odp" />
								</td>
								<td width="10%">ODP</td>
							</tr>
							<tr class="captioninfo">
				...
				<h3>Diese Erhebung anhand der Dateiendung dient der KOST und der
					PreservationPlanning-Expertengruppe der KOST. Dies bietet die
					Möglichkeit, zu erfahren welche Dateiformate in welcher Häufigkeit
					im content-Ordner der validierten SIPs enthalten sind und kann
					entsprechend der Weiterentwicklung von KOST-Val (zusätzliche
					Dateiformate) als auch eines systematischen PreservationPlannings
					mit einem Risikobasierten Ansatz verwendet werden.</h3>
				<h3>Wir sind Ihnen dankbar, wenn Sie diese KaD-Diagnosedatei auf
					Anfrage der KOST-Geschäftsstelle (kost-val@kost-ceco.ch) zusenden
					könnten.</h3>
			</body>
		</html>
	</xsl:template>
