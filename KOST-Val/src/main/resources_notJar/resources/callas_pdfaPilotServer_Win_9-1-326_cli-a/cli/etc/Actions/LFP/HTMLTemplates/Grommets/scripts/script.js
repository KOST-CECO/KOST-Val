...
function imagesLoaded() {
	...
	for( var thePageIndex = 0; thePageIndex < pcGetNumberOfPages(); thePageIndex++ ) {
		...
		if (theCalculateBySpacing) {
			...
			// into account
			var theRealSpacingHorizontal = (theSpacingHorizontal > 0) ? theSpacingHorizontal / theUserUnit : theSpacingHorizontal;
			var theRealSpacingVertical = (theSpacingVertical > 0) ? theSpacingVertical / theUserUnit : theSpacingVertical;
			...
			if (theNumberHorizontal < 0)
				theNumberHorizontal = (theRealSpacingHorizontal < 1) ? 0 : Math.round( theTrimboxWidth / theRealSpacingHorizontal );
			if (theNumberVertical < 0)
				theNumberVertical = (theRealSpacingVertical < 1) ? 0 : Math.round( theTrimboxHeight / theRealSpacingVertical );
		}
		...
		addGrommets( $( '#template_grommet'), theNumberHorizontal, theNumberVertical, thePageIndex,
					 theDistanceTop, theDistanceBottom, theDistanceLeft, theDistanceRight, theScalingForGrommet );
...
function cchipPrintLoop() {
    ...
    if (theTemplateGrommet) {
		...
		theTemplateGrommetPath = encodeURI('file://' + theTemplateGrommetPath).replace(/\#/g, "%23").replace(/\?/g, "%3F");
...
function addGrommets( inTemplateGrommet, inNumberHorizontal, inNumberVertical, inPageIndex,
					  theDistanceTop, theDistanceBottom, theDistanceLeft, theDistanceRight, inScaling ) {
	...
	for (var theIndexY = 0; theIndexY < inNumberVertical; theIndexY++) {
		...
		addGrommet( inTemplateGrommet, 'grommet_' + theGrommetIndex, pcAnchorPoints.centerMiddle, inPageIndex,
					pcPagebox.trimbox, pcAnchorPoints.leftTop, 
					theDistanceLeft, theDistanceTop + (theIndexY * theDeltaY), inScaling );
		theGrommetIndex += 1;

		// Add a grommet vertically along the right border
		addGrommet( inTemplateGrommet, 'grommet_' + theGrommetIndex, pcAnchorPoints.centerMiddle, inPageIndex,
					pcPagebox.trimbox, pcAnchorPoints.leftTop, 
					theTrimboxWidth - theDistanceRight, theDistanceTop + (theIndexY * theDeltaY), inScaling );
		theGrommetIndex += 1;
	}
	...
	for (var theIndexX = 1; theIndexX < (inNumberHorizontal-1) ; theIndexX++) {
		...
		addGrommet( inTemplateGrommet, 'grommet_' + theGrommetIndex, pcAnchorPoints.centerMiddle, inPageIndex,
					pcPagebox.trimbox, pcAnchorPoints.leftTop, 
					theDistanceLeft + (theIndexX * theDeltaX), theDistanceTop, inScaling );
		theGrommetIndex += 1;

		// Add a grommet vertically along the right border
		addGrommet( inTemplateGrommet, 'grommet_' + theGrommetIndex, pcAnchorPoints.centerMiddle, inPageIndex,
					pcPagebox.trimbox, pcAnchorPoints.leftTop, 
					theDistanceLeft + (theIndexX * theDeltaX), theTrimboxHeight - theDistanceBottom, inScaling );
		theGrommetIndex += 1;
	}
}
