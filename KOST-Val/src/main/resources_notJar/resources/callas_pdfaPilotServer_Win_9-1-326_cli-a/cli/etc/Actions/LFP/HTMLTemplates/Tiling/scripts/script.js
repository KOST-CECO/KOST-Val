...
function imagesLoaded() {
    ...
    if ((cchip.user.config.count_horizontal < 0) || (cchip.user.config.count_vertical < 0)) {
        ...
        globalCountHorizontal = (theSizeHorizontal == 0 ) ? 1 : Math.ceil( (globalTrimboxWidth / theSizeHorizontal) );
        globalCountVertical = (theSizeVertical == 0) ? 1 : Math.ceil( (globalTrimboxHeight / theSizeVertical) );
...
function getImageURL() {
    ...
    return encodeURI('file://' + pathName).replace(/\#/g, "%23").replace(/\?/g, "%3F");
}
