...
function imagesLoaded() {
	...
	var theVariableAddTotalNumberOfPages = getVariableValue( 'add_total_number_of_pages' );
	var theVariableAddTotalNumberOfPagesDivider = getVariableValue( 'add_total_number_of_pages_divider' );
	...
	for( var thePageIndex = theLowerLimit; thePageIndex < theUpperLimit; thePageIndex++ ) {
		...
		// Format the text we're adding
		theVariableText = (thePageIndex + 1).toString();
		if(theVariableAddTotalNumberOfPages == 1) {
			theVariableText += (theVariableAddTotalNumberOfPagesDivider + cals_doc_info.pages.length);
		}
