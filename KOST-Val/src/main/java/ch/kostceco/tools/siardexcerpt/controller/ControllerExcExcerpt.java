...
public class ControllerExcExcerpt implements MessageConstants
{
	...
	public static boolean mainExcerpt( String[] args ) throws IOException
	{
		 ...
		 */
		ControllerExcExcerpt controllerExcExcerpt = (ControllerExcExcerpt) context
				.getBean( "controllerExcExcerpt" );
		...
		if ( args.length == 4 ) {
			System.out.println( controllerExcExcerpt.getTextResourceServiceExc()
		...
		Map<String, String> configMap = controllerExcExcerpt
		...
		if ( !siardDatei.isDirectory() ) {
			...
			if ( !siardDateiNew.exists() ) {
				System.out.println(
						controllerExcExcerpt.getTextResourceServiceExc()
		...
		String outFileName = logConfigurator
				.configure( directoryOfOutput.getAbsolutePath(), outDateiName );
		File outFile = new File( outFileName );
		// Ab hier kann ins Output geschrieben werden...
		/*
		 * BufferedWriter out = new BufferedWriter( new OutputStreamWriter( new
		 * FileOutputStream( new File( outFileName + "_new.xml" ) ), "UTF-8" )
		 * ); try {
		 */
		...
		if ( !xslOrig.exists() || pathToXSL.equals( "(.. )" ) ) {
			...
			File xsltxt = new File(
					"resources" + File.separator + "SIARDexcerptAutoXSL.txt" );
			if ( !xsltxt.exists() ) {
				System.out.println( controllerExcExcerpt
						.getTextResourceServiceExc().getText( locale,
								EXC_ERROR_CONFIGFILE_FILENOTEXISTING,
								xsltxt.getAbsolutePath() ) );
				return false;
			}
			xslCopy = new File( directoryOfOutput.getAbsolutePath()
					+ File.separator + "SIARDexcerptAutoXSL_"
			...
			if ( !xslCopy.exists() ) {
				 ...
				 * 
				 * AUTO_XSL_TABLE_START -> Fuer jede Tabelle anlegen {0} ist die
				 * Zahl z.B. 1 fuer table1 AUTO_XSL_COLUMN -> Fuer jede Zeile
				 * anlegen {0} ist die columnNr z.B. 3 fur c3 AUTO_XSL_TABLE_END
				 * -> am Ende der Tabelle Anlegen. Dann wiederholen der ersten
				 * drei Schritte oder AUTO_XSL_FOOTER zum Abschluss anlegen
				 */
				...
				try {
					...
					InputStream inputStreamConfig = new FileInputStream(
							xmlExtracted );
					Reader readerConfig = new InputStreamReader(
							inputStreamConfig, "UTF-8" );
					...
					Util.oldnewstring( provEndXSL,
							controllerExcExcerpt.getTextResourceServiceExc()
									.getText( locale, EXC_AUTO_XSL_FOOTER ),
							xslCopy );
				} catch ( Exception e ) {
					Logtxt.logtxt( outFile, "<Error>" + controllerExcExcerpt
							.getTextResourceServiceExc().getText( locale,
			...
			xslCopy = new File( directoryOfOutput.getAbsolutePath()
					+ File.separator + xslOrig.getName() );
			if ( !xslCopy.exists() ) {
		...
		try {
			...
			NodeList nlDataOriginTimespan = doc
					.getElementsByTagName( "dataOriginTimespan" );
			Node nodeDataOriginTimespan = nlDataOriginTimespan.item( 0 );
			dataOriginTimespan = nodeDataOriginTimespan.getTextContent();

			NodeList nlSiardArchive = doc
					.getElementsByTagName( "siardArchive" );
			...
			Logtxt.logtxt( outFile,
					"<Error>" + controllerExcExcerpt.getTextResourceServiceExc()
							.getText( locale, EXC_ERROR_XML_UNKNOWN,
									e.getMessage() ) );
			Logtxt.logtxt( outFile,
					controllerExcExcerpt.getTextResourceServiceExc()
		...
		Logtxt.logtxt( outFile, controllerExcExcerpt.getTextResourceServiceExc()
				.getText( locale, EXC_MESSAGE_XML_HEADER, xslCopy.getName() ) );
		Logtxt.logtxt( outFile, controllerExcExcerpt.getTextResourceServiceExc()
				.getText( locale, EXC_MESSAGE_XML_START, ausgabeStart ) );
		Logtxt.logtxt( outFile, controllerExcExcerpt.getTextResourceServiceExc()
				.getText( locale, EXC_MESSAGE_XML_TEXT, archive, "Archive" ) );
		Logtxt.logtxt( outFile, controllerExcExcerpt.getTextResourceServiceExc()
				.getText( locale, EXC_MESSAGE_XML_TEXT, dbname, "dbname" ) );
		Logtxt.logtxt( outFile,
				controllerExcExcerpt.getTextResourceServiceExc().getText(
						locale, EXC_MESSAGE_XML_TEXT, dataOriginTimespan,
						"dataOriginTimespan" ) );
		Logtxt.logtxt( outFile,
				controllerExcExcerpt.getTextResourceServiceExc().getText(
						locale, EXC_MESSAGE_XML_TEXT, dbdescription,
						"dbdescription" ) );
		Logtxt.logtxt( outFile,
				controllerExcExcerpt.getTextResourceServiceExc().getText(
						locale, EXC_MESSAGE_XML_TEXT, keyexcerpt,
						"keyexcerpt" ) );
		Logtxt.logtxt( outFile, controllerExcExcerpt.getTextResourceServiceExc()
				.getText( locale, EXC_MESSAGE_XML_INFO ) );
		...
		if ( !okC ) {
			...
			Logtxt.logtxt( outFile,
					controllerExcExcerpt.getTextResourceServiceExc().getText(
							locale, EXC_ERROR_XML_C_CANNOTEXTRACTRECORD ) );
			Logtxt.logtxt( outFile,
					controllerExcExcerpt.getTextResourceServiceExc()
							.getText( locale, EXC_MESSAGE_XML_LOGEND ) );
			System.out.println(
					controllerExcExcerpt.getTextResourceServiceExc().getText(
							locale, EXC_MESSAGE_C_EXCERPT_NOK, outFileName ) );
			 ...
			 * DocumentBuilderFactory.newInstance(); factory.setValidating(
			 * false );
			 * 
			 * factory.setExpandEntityReferences( false );
			 * 
			 * Document docConfig = factory.newDocumentBuilder().parse(
			 * configFile ); NodeList list = docConfig.getElementsByTagName(
			 * "configuration" ); Element element = (Element) list.item( 0 );
			 * 
			 * Document docLog = factory.newDocumentBuilder().parse( outFile );
			 * 
			 * Node dup = docLog.importNode( element, true );
			 * 
			 * docLog.getDocumentElement().appendChild( dup ); FileWriter writer
			 * = new FileWriter( outFile );
			 * 
			 * ByteArrayOutputStream baos = new ByteArrayOutputStream();
			 * ElementToStream( docLog.getDocumentElement(), baos ); String
			 * stringDoc2 = new String( baos.toByteArray() ); writer.write(
			 * stringDoc2 ); writer.close();
			 * 
			 * // Der Header wird dabei leider verschossen, wieder zurueck
			 * aendern String newstring =
			 * controllerExcExcerpt.getTextResourceServiceExc().getText(
			 ...
			 * controllerExcExcerpt.getTextResourceServiceExc().getText(
			 * ERROR_XML_UNKNOWN, e.getMessage() ) ); System.out.println(
			...
			// Record konnte extrahiert werden
			System.out.println( controllerExcExcerpt.getTextResourceServiceExc()
					.getText( locale, EXC_MESSAGE_C_EXCERPT_OK, outFileName ) );
			excerpt = true;
			return excerpt;
