...
public class ControllerExcExcerpt implements MessageConstants
{
	...
	public static boolean mainExcerpt( String[] args ) throws IOException
	{
		...
		if ( !okC ) {
			 ...
			 * 
			 * docLog.getDocumentElement().appendChild( dup ); FileWriter writer = new FileWriter( outFile
			 * );
			 * 
			 ...
			 * 
			 * // Der Header wird dabei leider verschossen, wieder zurueck aendern String newstring =
			 * controllerExcExcerpt.getTextResourceServiceExc().getText( MESSAGE_XML_HEADER,
			 * xslCopy.getName() ); String oldstring =
			 * "<?xml version=\"1.0\" encoding=\"UTF-8\"?><table>"; Util.oldnewstring( oldstring,
			 * newstring, outFile );
			 * 
			 * } catch ( Exception e ) { LOGGER.logError( "<Error>" +
			 * controllerExcExcerpt.getTextResourceServiceExc().getText( ERROR_XML_UNKNOWN, e.getMessage()
			 * ) ); System.out.println( "Exception: " + e.getMessage() ); } */
