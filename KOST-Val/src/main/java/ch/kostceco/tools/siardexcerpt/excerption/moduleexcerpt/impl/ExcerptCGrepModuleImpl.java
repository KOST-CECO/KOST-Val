 ...
 * anzeigen
 */
public class ExcerptCGrepModuleImpl extends ValidationModuleImpl
		implements ExcerptCGrepModule
{
			...
			throws ExcerptCGrepException
	{
		...
		File xmlExtracted = new File( siardDatei.getAbsolutePath()
		...
		try {
			if ( tempOutFileMt.exists() ) {
				 ...
				 * 
				 * wird nicht verwendet, da es jetzt gelöscht werden muss und
				 * nicht spätestens bei exit. wenn es nicht gelöchscht werden
				 * kann wird es geleert.
				 */
			}
			...
			if ( folder.startsWith( "Configuration-Error:" ) ) {
				Logtxt.logtxt( outFile, getTextResourceServiceExc()
						.getText( locale, EXC_MESSAGE_XML_MODUL_B ) + folder );
				return false;
			}
			if ( cell.startsWith( "Configuration-Error:" ) ) {
				Logtxt.logtxt( outFile, getTextResourceServiceExc()
						.getText( locale, EXC_MESSAGE_XML_MODUL_B ) + cell );
				return false;
			}
			String tabfolder = "";
			String tabname = "";
			...
			try {
				/*
				 * Der excerptString kann Leerschläge enthalten, welche bei grep
				 * Problem verursachen. Entsprechend werden diese durch .
				 * ersetzt (Wildcard)
				 */
				...
				dbf.setFeature( "http://xml.org/sax/features/namespaces",
						false );

				NodeList nlTable = doc.getElementsByTagName( "table" );
				for ( int i = 0; i < nlTable.getLength(); i++ ) {
					/*
					 * cellname und celldescription leeren, da vorgängige
					 * Tabelle nicht die Richtige war.
					 */
					cellname = "";
					celldescription = "";
					...
					for ( int x = 0; x < childNodes.getLength(); x++ ) {
						...
						if ( subNodeI.getNodeName().equals( "folder" ) ) {
							...
							if ( subNodeI.getTextContent().equals( folder ) ) {
								/*
								 * Es ist die richtige Tabelle. Ensprechend wird
								 * i ans ende Gestellt, damit die i-Schlaufe
								 * beendet wird.
								 */
						...
						} else if ( subNodeI.getNodeName()
								.equals( "description" ) ) {
							tabdescriptionProv = new String(
									subNodeI.getTextContent() );
							// Umlaute konnte nicht korrekt in UTF-8
							// wiedergegeben werden --> normalisieren
							tabdescriptionProv = Util
									.umlaute( tabdescriptionProv );
							tabdescriptionProv = new String(
									tabdescriptionProv.getBytes(),
									StandardCharsets.UTF_8 );
							/*
							 * in der description generiert mit csv2siard wird
							 * nach "word" der Select Befehl angehängt. Dieser
							 * soll nicht mit ausgegeben werden.
							 */
							String word = "\\u000A";
							...
							// "+tabdescription );
								...
								.equals( "columns" ) ) {
							NodeList childNodesColumns = subNodeI
									.getChildNodes();
							int counter = 0;
							for ( int y = 0; y < childNodesColumns
									.getLength(); y++ ) {
								Node subNodeII = childNodesColumns.item( y );
								if ( subNodeII.getNodeName()
										.equals( "column" ) ) {
									...
									for ( int z = 0; z < childNodesColumn
										...
										if ( subNodeIII.getNodeName()
											...
											String celldescriptionProv = new String(
													subNodeIII
															.getTextContent() );
											celldescriptionProv = Util.umlaute(
													celldescriptionProv );
				...
				Logtxt.logtxt( outFile, getTextResourceServiceExc().getText(
						locale, EXC_MESSAGE_XML_TEXT, tabname, "tabname" ) );
				Logtxt.logtxt( outFile,
						getTextResourceServiceExc().getText( locale,
								EXC_MESSAGE_XML_TEXT,
								schemafolder + "/" + tabfolder, "tabfolder" ) );
				Logtxt.logtxt( outFile,
						getTextResourceServiceExc().getText( locale,
								EXC_MESSAGE_XML_TEXT, tabdescription,
								"tabdescription" ) );
				Logtxt.logtxt( outFile, getTextResourceServiceExc().getText(
						locale, EXC_MESSAGE_XML_TEXT, cellname, "name" ) );
				Logtxt.logtxt( outFile,
						getTextResourceServiceExc().getText( locale,
								EXC_MESSAGE_XML_TEXT, celldescription,
								"description" ) );

				// grep "<c11>7561234567890</c11>" table13.xml >> output.txt
				String resultExec = Grep.execGrep( " -E ", excerptStringM,
						fMaintable, tempOutFileMt, workDir, dirOfJarPath );

				if ( !resultExec.equals( "OK" ) ) {
					// grep hat nicht funktioniert
					Logtxt.logtxt( outFile, getTextResourceServiceExc()
							.getText( locale, EXC_MESSAGE_XML_MODUL_C )
							+ getTextResourceServiceExc().getText( locale,
				...
				Logtxt.logtxt( outFile, getTextResourceServiceExc().getText(
						locale, EXC_MESSAGE_XML_ELEMENT_CONTENT, content ) );
				Logtxt.logtxt( outFile,
						getTextResourceServiceExc().getText( locale,
								EXC_MESSAGE_XML_ELEMENT_CLOSE,
								schemaname.replace( " ", "" ) + "_"
										+ name.replace( " ", "" ) ) );
				...
				if ( tempOutFileMt.exists() ) {
					 ...
					 * Util.deleteDir( tempOutFile );
					 * 
					 * wird nicht verwendet, da es jetzt gelöscht werden muss
					 * und nicht spätestens bei exit. wenn es nicht gelöchscht
					 * werden kann wird es geleert.
					 */
				}
				content = "";
		...
		try {
			...
			for ( int j = 1; j < 21; j++ ) {
				...
				if ( folder.equals( "(..)" ) ) {
					...
					File fSchema = new File( siardDatei.getAbsolutePath()
							+ File.separator + "content" + File.separator
							+ schemafolder );
					File fSubtable = new File(
							fSchema.getAbsolutePath() + File.separator + folder
									+ File.separator + folder + ".xml" );
					File fSubtableTemp1 = new File(
							fSchema.getAbsolutePath() + File.separator + folder
									+ File.separator + folder + "_Temp1.xml" );
					File fSubtableTemp2 = new File(
							fSchema.getAbsolutePath() + File.separator + folder
									+ File.separator + folder + "_Temp2.xml" );
					File fSubtableTemp3 = new File(
							fSchema.getAbsolutePath() + File.separator + folder
									+ File.separator + folder + "_Temp3.xml" );
					/*
					 * mit Util.oldnewstring respektive replace können sehr
					 * grosse files nicht bearbeitet werden!
					 * 
					...
					if ( sed.equalsIgnoreCase( "yes" ) ) {
						...
						// fSearchtableTemp
						String resultExec = Sed.execSed( sed1, fSubtable,
								fSubtableTemp1, workDir, dirOfJarPath );
						Thread.sleep( sleepLong );
						if ( resultExec.equals( "OK" ) ) {
							resultExec = Sed.execSed( sed2, fSubtableTemp1,
									fSubtableTemp2, workDir, dirOfJarPath );
							Thread.sleep( sleepLong );
							if ( resultExec.equals( "OK" ) ) {
								resultExec = Sed.execSed( sed3, fSubtableTemp2,
										fSubtableTemp3, workDir, dirOfJarPath );
								Thread.sleep( sleepLong );
								if ( resultExec.equals( "OK" ) ) {
									resultExec = Sed.execSed( sed4,
											fSubtableTemp3, fSubtable, workDir,
											dirOfJarPath );
									Thread.sleep( sleepLong );
								}
							}
					...
					try {
						/*
						 * Der excerptString kann Leerschläge enthalten, welche
						 * bei grep Problem verursachen. Entsprechend werden
						 * diese durch . ersetzt (Wildcard)
						 */
						String excerptStringM = excerptString.replaceAll( " ",
								"." );
						excerptStringM = excerptStringM.replaceAll( "\\.",
								"\\.*" );
						excerptStringM = "<" + cell + ">" + excerptStringM
								+ "</" + cell + ">";
						...
						DocumentBuilder db = dbf.newDocumentBuilder();
						Document doc = db
								.parse( new FileInputStream( xmlExtracted ) );
						doc.getDocumentElement().normalize();
						dbf.setFeature(
						...
						for ( int i = 0; i < nlTable.getLength(); i++ ) {
							...
							for ( int x1 = 0; x1 < nlSchemaChild
								...
								if ( subNode.getNodeName()
									...
									if ( subNode.getTextContent()
											.equals( schemafolder ) ) {
										tabschema = subNode.getTextContent();
										// das richtige schema
										NodeList childNodes = nodenlTable
												.getChildNodes();
										for ( int x = 0; x < childNodes
												.getLength(); x++ ) {
											Node subNodeI = childNodes
													.item( x );
											if ( subNodeI.getNodeName()
													.equals( "folder" ) ) {
												// System.out.println(
												// subNodeI.getTextContent() );
												if ( subNodeI.getTextContent()
														.equals( folder ) ) {
													/*
													 * Es ist die richtige
													 * Tabelle. Ensprechend wird
													 * i ans ende Gestellt,
													 * damit die i-Schlaufe
													 * beendet wird.
													 */
													tabfolder = subNodeI
															.getTextContent();
												...
												tabdescriptionProv = new String(
														...
														StandardCharsets.UTF_8 );
												/*
												 * in der description generiert
												 * mit csv2siard wird nach
												 * "word" der Select Befehl
												 * angehängt. Dieser soll nicht
												 * mit ausgegeben werden.
												 */
												String word = "\\u000A";
												int endIndex = tabdescriptionProv
														.indexOf( word );
												if ( endIndex == -1 ) {
													tabdescription = tabdescriptionProv;
												} else {
							...
							if ( i == nlTable.getLength() ) {
								...
								Logtxt.logtxt( outFile,
										getTextResourceServiceExc().getText(
												locale, EXC_MESSAGE_XML_TEXT,
												tabname, "tabname" ) );
								Logtxt.logtxt( outFile,
										getTextResourceServiceExc().getText(
												locale, EXC_MESSAGE_XML_TEXT,
						...
						// output.txt
						String resultExec = Grep.execGrep( " -E ",
								excerptStringM, fSubtable, tempOutFile, workDir,
								dirOfJarPath );
						...
						Logtxt.logtxt( outFile,
								getTextResourceServiceExc().getText( locale,
										EXC_MESSAGE_XML_ELEMENT_CONTENT,
										content ) );
						Logtxt.logtxt( outFile,
								getTextResourceServiceExc().getText( locale,
										EXC_MESSAGE_XML_ELEMENT_CLOSE,
						...
						if ( tempOutFile.exists() ) {
							 ...
							 * Util.deleteDir( tempOutFile );
							 * 
							 * wird nicht verwendet, da es jetzt gelöscht werden
							 * muss und nicht spätestens bei exit. wenn es nicht
							 * gelöchscht werden kann wird es geleert.
							 */
						}
						content = "";
