...
public class ExcerptAConfigModuleImpl extends ValidationModuleImpl implements ExcerptAConfigModule {
	...
	public boolean validate(File siardDatei, File configFileHard, String inputMainname, Map<String, String> configMap,
		...
		try {
			 ...
			 * 
			 * Wenn mainname && mainname exist => Haupttabelle = mainname (mainname,
			 * mainfolder, primarykeyname, primarykeycell, title) ausfuellen
			 * 
			 * -> dann wenn foreignKey = primaryKey.maintable subtables (name, folder,
			 ...
			 * count primaryKey
			 * 
			 * if pkInt = 1 dann config maintable (mainname, mainfolder, primarykeyname,
			 * primarykeycell, title) ausfuellen
			 * 
			 * -> dann wenn foreignKey = primaryKey subtables (name, folder, foreignkeycell)
			 * ausfuellen
			 * 
			 * if pkInt > 1 dann analog oben aber mit dieser, welche am meisten verwendet
			 * wird
			 * 
			 * if pkInt = 0 dann jene Tablle mit den meisten column. Keine Verknuepfung mit
			 * anderen Tabellen.
			 *
			 * Die 12 searchCells definieren: Die erste ist der pk. Die restlichen werden
			 * mit ersten 11 respektive 12 sinnvollen Spalten definiert und erstellt.
			 * Sinnvoll bedeutet, dass die verschiedenen types priorisiert werden. (nur wenn
			 * mehr als 12 column in maintable)
			 * 
			 ...
			 * 
			 * 5 fuer "TIME", "TIME WITH TIME ZONE", "TIMESTAMT" und "TIMESTAMP WITH TIME
			 * ZONE".
			 * 
			 * 6 fuer "BINARY VARYING", "BINARY", "BIT VARYING", "BIT" und "XML".
			 * 
			 * 7 fuer "BINARY LARGE OBJECT", "BOOLEAN", "CHARACTER LARGE OBJECT", "NATIONAL
			 * CHARACTER LARGE OBJECT" und ggf andere
			 */
			...
			try {
				...
				for (int x = 0; x < nlColumns.getLength(); x++) {
					 ...
					 * number.setTextContent( "c" + (1) ); nodeColumns.appendChild(number);
					 */
				...
				if (!inputMainname.equals("(..)")) {
					...
					/*
					 * Kontrollieren ob inputMainname existiert. Wenn nicht dann false und gleich
					 * wie (..) weiter fahren
					 */
					...
					for (int y = 0; y < nlChildDetailTable.getLength(); y++) {
						...
						if (nodeSubDetailTable.getNodeName().equals("columns")) {
							...
							for (int zC = 0; zC < nlColumnsMT.getLength(); zC++) {
								...
								if (!numberProv.equals("") || !nameProv.equals("") || !typeProv.equals("")) {
									...
									if (numberProv.equals(primarykeycell)) {
										...
										numberProv = "";
										typeProv = "";
									} else if (typeProv.startsWith("DOUBLE PRECISION") || typeProv.startsWith("FLOAT")
											|| typeProv.startsWith("INTERVAL") || typeProv.startsWith("REAL")) {
							...
							/*
							 * die verschiedenen Prio zusammenfassen. p1 mit p2 erweitern, dann p3, dann
							 * p4...
							 * 
				...
				if (!boolMainname) {
					...
					if (nlPK.getLength() == 0) {
						/*
						 * kein Primaerschluessel. if pkInt = 0 dann jene Tabelle mit den meisten
						 * column. Der Schluessel ist die erste Spalte, welche nicht Nullable sein darf
						 * oder die erste. Keine Verknuepfung mit anderen Tabellen.
						 */
						int tableX2 = 0;
						int tableColNum = 0;
						...
						for (int zC = 0; zC < nlmostColumn.getLength(); zC++) {
							...
							if (!numberProv.equals("") || !nameProv.equals("") || !typeProv.equals("")) {
								...
								if (numberProv.equals(primarykeycell)) {
									...
									numberProv = "";
									typeProv = "";
								} else if (typeProv.startsWith("BINARY LARGE OBJECT") || typeProv.startsWith("BOOLEAN")
										|| typeProv.startsWith("CHARACTER LARGE OBJECT")
										|| typeProv.startsWith("NATIONAL CHARACTER LARGE OBJECT")) {
									// 72-83 = Prio7
									for (int z = 72; z < 84; z++) {
									...
									numberProv = "";
									typeProv = "";
								} else if (typeProv.startsWith("CHARACTER VARYING") || typeProv.startsWith("CHARACTER")
						...
						if (nlPK.getLength() > 1 && nlFK.getLength() > 0) {
							 ...
							 * Haelfte bestimmen. Wert der 1. Referenced Tablename herauslesen & zaehler =1.
							 * Dann der zweite Wert herauslesen, wenn gleich zaehler erhoehen, ansonsten nr
							 * merken. Den 3. und ff Wert herauslesen und ggf zaehler erhoehen. Wenn Zaehler
							 * am Ende Hoeher/= der haelfte ist, ist es der meistverwendete
							 * Primaerschluessel. Ansonsten wert speichern und mit anderen Werten gleich
							 * weiterfahren. Wenn keiner die Haelfte erreicht, herausfinder welcher die
							 * meisten treffer hat.
							 */
				 ...
				 * 
				 * <subtable1> <st1keyname>(..)</st1keyname><st1name>(..)</st1name><
				 * st1folder>(..)</st1folder ><st1fkcell>(..)</st1fkcell> </subtable1>
				 * <subtables>(..)</subtables> mit Werte ersetzten.
				 * 
				 * Wenn mainname = referencedTable und referenced = primarykeyname, dann
				 * ermitteln, welcher name und folder diese Tabelle hat und die Zellnummer der
				 * reference-column.
				 * 
				 * Das fuer jede Tabelle wiederholen, zu einem Wert zusammensetzten und
				 * ersetzen.
				 */
									...
									for (int y1 = 0; y1 < childNodesColumns.getLength(); y1++) {
										...
										for (int y2 = 0; y2 < childNodesColumn.getLength(); y2++) {
											...
											if (column && subKeyCellProv != "") {
												...
												subName = " <st" + stcounter + "keyname>" + subKeyName + "</st"
														+ stcounter + "keyname><st" + stcounter + "name>" + subName
														+ "</st" + stcounter + "name>";
												subFolder = "<st" + stcounter + "folder>" + subFolder + "</st"
														+ stcounter + "folder>";
												...
												subFolderSchema = "<st" + stcounter + "schemafolder>" + subFolderSchema
														...
														+ "schemaname>" + subNameSchema + "</st" + stcounter
														+ "schemaname>";
												/*
												 * subName = " <subtable><keyname>" + subKeyName + "</keyname><name>" +
												 * subName + "</name>"; subFolder = "<folder>" + subFolder +
												 * "</folder>"; subKeyCell = "<foreignkeycell>" + subKeyCell +
												...
												String subtablesNo = "<st" + stcounter + "keyname>(..)</st" + stcounter
														+ "keyname><st" + stcounter + "name>(..)</st" + stcounter
														+ "name><st" + stcounter + "folder>(..)</st" + stcounter
														+ "folder><st" + stcounter + "fkcell>(..)</st" + stcounter
														+ "fkcell><st" + stcounter + "schemafolder>(..)</st" + stcounter
														+ "schemafolder><st" + stcounter + "schemaname>(..)</st"
														+ stcounter + "schemaname>";
