...
public class ValidationAvalidationMkvModuleImpl extends ValidationModuleImpl implements ValidationAvalidationMkvModule {
		...
		if (!checkTool.equals("OK")) {
			...
			try {
				...
				if (!resultExec.equals("OK") || !outputProbe.exists()) {
					...
					while (scanner.hasNextLine()) {
						...
						if (line.contains("codec_name=")) {
							...
							if (line.contains(ffv1Codec) && ffv1Config.equals("FFV1")) {
								// OK
								type = "videocodec";
								formatCodec = formatCodec + type + "=" + ffv1Config + "  ";
								countVideoCodec = countVideoCodec + 1;
							} else if (line.contains(avcCodec) && avcConfig.equals("AVC")) {
								// OK
								type = "videocodec";
								...
								// OK
								type = "audiocodec";
								formatCodec = formatCodec + type + "=" + flacConfig + "  ";
					...
					Logtxt.logtxt(logFile,
							getTextResourceService().getText(locale, MESSAGE_XML_MODUL_A_MKV) + getTextResourceService()
					...
					if (countVideoCodec == 0) {
						if (configMap.get("Allowedmkvnovideo").equals("Error")) {
							...
							Logtxt.logtxt(logFile,
									getTextResourceService().getText(locale, MESSAGE_XML_MODUL_A_MKV)
											+ getTextResourceService().getText(locale,
													ERROR_XML_A_VIDEO_CODEC_NOVIDEO_WARNING, "MKV"));
						}
					}
					if (countAudioCodec == 0) {
						if (configMap.get("Allowedmkvnoaudio").equals("Error")) {
							...
							Logtxt.logtxt(logFile,
									getTextResourceService().getText(locale, MESSAGE_XML_MODUL_A_MKV)
											+ getTextResourceService().getText(locale,
													ERROR_XML_A_VIDEO_CODEC_NOAUDIO_WARNING, "MKV"));
						}
					}
