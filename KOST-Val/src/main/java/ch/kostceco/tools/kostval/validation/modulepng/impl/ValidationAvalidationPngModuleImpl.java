...
public class ValidationAvalidationPngModuleImpl extends ValidationModuleImpl implements ValidationAvalidationPngModule {

	private boolean min = false;

	@Override
	public boolean validate(File valDatei, File directoryOfLogfile, Map<String, String> configMap, Locale locale,
			File logFile, String dirOfJarPath) throws ValidationApngvalidationException {
		...
		try {
			...
			if (!resultExec.equals("OK")) {
				...
				if (min) {
					...
					} else if (msg.startsWith("invalid number of") || msg.startsWith(" invalid number of")
					...
					} else if (msg.startsWith("invalid") || msg.startsWith(" invalid") || msg.startsWith("  invalid")) {
						// invalid bKGD length
						msg = msg + ".";
						if (locale.toString().startsWith("de")) {
					...
					Logtxt.logtxt(logFile,
							modul + getTextResourceService().getText(locale, MESSAGE_XML_SERVICEMESSAGE, msg, ""));
					isValid = false;
				}
			} else {
				// OK
