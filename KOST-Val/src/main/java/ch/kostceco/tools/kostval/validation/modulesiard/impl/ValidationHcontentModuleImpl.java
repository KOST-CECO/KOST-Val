...
public class ValidationHcontentModuleImpl extends ValidationModuleImpl implements ValidationHcontentModule {
	...
	public boolean validate(File valDatei, File directoryOfLogfile, Map<String, String> configMap, Locale locale,
		...
		try {
			/*
			 * Extract the metadata.xml from the temporary work folder and build a jdom
			 * document
			 */
												...
												if (!resultExec.equals("OK")) {
													...
													if (min) {
														...
														if (onWorkConfig.equals("yes")) {
															...
															int count = StringUtils.countOccurrencesOf(resultExec,
																	searchString);
															String resultExecFirst = resultExec;
															if (count < 1) {
																resultExecFirst = resultExec;
															} else {
																String[] resultExecArray = StringUtils.split(resultExec,
																		searchString);
																resultExecFirst = resultExecArray[0]
																		+ getTextResourceService().getText(locale,
																				MESSAGE_XML_H_COUNT_ERRORS, count);
															}
															Logtxt.logtxt(logFile,
																	getTextResourceService().getText(locale,
																			MESSAGE_XML_MODUL_H_SIARD)
			...
			if (min) {
				...
				Logtxt.logtxt(logFile,
						getTextResourceService().getText(locale, MESSAGE_XML_MODUL_H_SIARD) + getTextResourceService()
			...
			if (min) {
				...
				Logtxt.logtxt(logFile,
						getTextResourceService().getText(locale, MESSAGE_XML_MODUL_H_SIARD) + getTextResourceService()
								.getText(locale, ERROR_XML_UNKNOWN, e.getMessage() + " (SAXException)"));
			}
		}
	 ...
	 * maxOccurs = unbounded the validation of the numbers can't been executed. A
	 * Warning is given.
	 */
	private boolean verifyRowCount(File xmlFile, File schemaLocation, Locale locale, File logFile)
		...
		if (range.min == 0 && range.max == UNBOUNDED) {
			...
			if (min) {
				...
				Logtxt.logtxt(logFile,
						getTextResourceService().getText(locale, MESSAGE_XML_MODUL_H_SIARD) + getTextResourceService()
								.getText(locale, MESSAGE_XML_H_TABLE_NOT_VALIDATED1, schemaLocation.getName()));
			}
			return true;
		} else {
			return true;
