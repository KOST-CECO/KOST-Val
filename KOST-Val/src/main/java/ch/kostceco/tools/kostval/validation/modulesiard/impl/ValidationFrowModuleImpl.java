 ...
 */
{
		...
		try {
			...
			if (min) {
				...
				Logtxt.logtxt(logFile,
						getTextResourceService().getText(locale, MESSAGE_XML_MODUL_F_SIARD) + getTextResourceService()
	...
	private boolean validateTable(Element table, Namespace ns, String pathToWorkDir, File schemaPath, Locale locale,
			File logFile) {
		boolean valid = true;
		boolean validR = true;
	...
	private boolean validateRow(File tableXsd, int rowmax, Locale locale, Boolean min, File logFile) {
		...
		try {
			...
			try {
				...
				Logtxt.logtxt(logFile,
						getTextResourceService().getText(locale, MESSAGE_XML_MODUL_F_SIARD) + getTextResourceService()
								.getText(locale, ERROR_XML_UNKNOWN, e.getMessage() + " (IOException)"));
			}
