...
public class Controllervalinitlog implements MessageConstants {
	...
	// TODO
	public boolean valInitlog(String[] args, Map<String, String> configMap, File directoryOfLogfile, Locale locale,
			String ausgabeStart, File logFile, String dirOfJarPath, String versionKostVal) throws IOException {
		...
		String val = getTextResourceService().getText(locale, MESSAGE_XML_VALTYPE, " " + formatValOn);
		String rec = getTextResourceService().getText(locale, MESSAGE_XML_AZTYPE, " " + formatRecOn);
		...
		Logtxt.logtxt(logFile, "<configuration>" + config + "</configuration>");
		...
		/*
		 * Java 64 kontrollieren, da es ja zwischenzeitlich geaendert haben koennte
		 */
		...
		// lengthNow );
		File PdfValidatorAPIDll64 = new File(dirOfJarPath + File.separator + "resources" + File.separator
				+ "3-Heights_PDF-Validator_dll" + File.separator + "x64" + File.separator + "PdfValidatorAPI.dll");
		...
		if (java64.contains("64")) {
			if (!lengthNow.equals(length64)) {
				if (!PdfValidatorAPIDll.getParentFile().canWrite()) {
					// kein Schreibrecht
					String warning64xml = getTextResourceService().getText(locale, WARNING_JARDIRECTORY_NOTWRITABLEXML,
							PdfValidatorAPIDll.getParentFile(), PdfValidatorAPIDll.getAbsolutePath(),
							PdfValidatorAPIDll64.getAbsolutePath(), "32", java64);
					Logtxt.logtxt(logFile, warning64xml);
					String warning64 = getTextResourceService().getText(locale, WARNING_JARDIRECTORY_NOTWRITABLE,
							PdfValidatorAPIDll.getParentFile(), PdfValidatorAPIDll.getAbsolutePath(),
							PdfValidatorAPIDll64.getAbsolutePath(), "32", java64);
