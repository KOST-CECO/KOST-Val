...
public class KOSTVal implements MessageConstants {
	...
	public static boolean main(String[] args, String versionKostVal, String cliOrGui) throws IOException {
		...
		Map<String, String> configMap = kostval.getConfigurationService().configMap(locale, logtype, valDatei,
				dirOfJarPath);
		...
		Controllervalinitlog controller0log = (Controllervalinitlog) context.getBean("controllervalinitlog");
		boolean valInitlog = controller0log.valInitlog(args, configMap, directoryOfLogfile, locale, ausgabeStart,
		...
		/*
		 * Ueberpruefung des optionalen Parameters (4 --max = xml + verbose --> im
		 * Verbose-mode werden die originalen Logs nicht geloescht (Jhove & Co.)
		 */
		...
		if (args[0].equalsIgnoreCase("--format")) {
			...
			if (configRepPdfa.contains("yes") && configRepPdfa2u.contains("yes")) {
				System.out.println("repairPdfa = true;");
				repairPdfa = true;
			}
			...
			if (!valDatei.isDirectory()) {
				 ...
				 * verbose, dirOfJarPath, configMap, context );
				 */
				...
				String valFile = controller1.valFoFile(valDatei, logFileName, directoryOfLogfile, verbose, dirOfJarPath,
						...
						fileToOutputStart, repairPdfa, repairSiard, repairXls, true);
			...
			if (ech0160validation.equals("no")) {
				...
				System.out.println(kostval.getTextResourceService().getText(locale, ERROR_CONFIG_SIP));
				// = SIP-Validierung in der Konfiguration ausgeschaltet.
				Logtxt.logtxt(logFile, "<Sip><Validation>");
				Logtxt.logtxt(logFile, kostval.getTextResourceService().getText(locale, MESSAGE_XML_VALTYPE, "SIP"));
				// = <ValType>Validierung: {0}</ValType>
				Logtxt.logtxt(logFile, "<ValFile>" + valDatei.getAbsolutePath() + "</ValFile>");
				Logtxt.logtxt(logFile, kostval.getTextResourceService().getText(locale, MESSAGE_XML_MODUL_Aa_SIP));
				// = <Error><Modul>1A) Lesbarkeit</Modul>
				Logtxt.logtxt(logFile, kostval.getTextResourceService().getText(locale, ERROR_XML_CONFIG_SIP));
			...
			/*
			 * Ueberpruefung des Parameters (Val-Typ): format / sip args[0] ist nicht
