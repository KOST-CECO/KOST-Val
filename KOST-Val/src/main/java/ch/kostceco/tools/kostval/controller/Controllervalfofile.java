...
public class Controllervalfofile implements MessageConstants
{
	...
	public String valFoFile( File valDatei, String logFileName,
			...
			Locale locale, File logFile,int countToValidated ) throws IOException
	{
		...
		try {
				if ( !valDatei.isDirectory() ) {
					...
					/*
					 * Ergebnis ist die Datei z.B. PDFA oder SIARD wenn
					 * einwandfrei erkannt ansonsten wird _ext angehängt, wenn
					 * die Dateiendung nicht stimmt z.B. JPEG_ext wenn eine
					 * .tiff-Datei als JPEG erkannt wird.
					 * 
					 * Wenn nichts erkannt wird UNKNOWN_DATEIENDUNG ausgegeben
					 * z.B. UNKNOWN_CDR
					 */
						...
						if ( recFormat.equals( "PDFA" ) ) {
							...
							if ( pdfaValidation.equals( "yes" ) ) {
								...
								if ( valFile ) {
									...
									return "countInvalid"; //countInvalid = countInvalid + 1;
								...
								Logtxt.logtxt( logFile, getTextResourceService()
										...
										+ getTextResourceService().getText(
												locale, MESSAGE_XML_AZTYPE,
												"PDF/A" )
										+ getTextResourceService().getText(
												locale, MESSAGE_XML_VALFILE,
												valDatei.getAbsolutePath() ) );
