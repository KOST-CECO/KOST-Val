...
public class ValidationMlobModuleImpl extends ValidationModuleImpl
		implements ValidationMlobModule
{
	...
	public boolean validate( File valDatei, File directoryOfLogfile,
			...
			String dirOfJarPath ) throws ValidationMlobException
	{
			...
			try {
				...
				if ( min ) {
					...
					Logtxt.logtxt( logFile, getTextResourceService()
							.getText( locale, MESSAGE_XML_MODUL_M_SIARD )
							+ getTextResourceService().getText( locale,
									ERROR_XML_UNKNOWN,
				...
				try {
					...
					for ( Iterator<String> iterator = filesInSiardKeys
						...
						if ( entryNameFile.isDirectory() ) {
							...
							if ( recFormat.contains( "_ext" ) ) {
								...
								if ( entryNameParent.getAbsolutePath()
										.equals( oldParentRec ) ) {
									...
									if ( !msgRec.equals( "" ) ) {
										String oldParentRecShort = oldParentRec
												.replace( pathToWorkDir, "" );
										...
										Logtxt.logtxt( logFile,
												getTextResourceService()
														...
														+ getTextResourceService()
																.getText(
																		locale,
																		MESSAGE_XML_M_AZ_INCORRECTFILE,
																		oldParentRecShort,
																		msgRec,
																		warning, warningEl ) );
										msgRec = "";
								...
								oldParentRec = entryNameParent
										.getAbsolutePath();
							} else if ( recFormat.contains( "UNKNOWN_" )
									|| recFormat.contains( "_" ) ) {
								...
								if ( entryNameParent.getAbsolutePath()
									...
									counterDetailDirUnknown = counterDetailDirUnknown
											+ 1;
								} else {
									// neuer Ordner
									...
									msgUnknown = msgUnknown
											+ " </Message><Message> - "
											+ entryNameParent.getAbsolutePath()
													.replace( pathToWorkDir,
															"" )
											+ ": " + entryNameFile.getName();
								}
								oldParentUnknown = entryNameParent
										.getAbsolutePath();
							} else {
								// Exakte Dateienduung
								/*
								 * System.out.println(
								 * "Exakte Dateiendung der LOB-Datei: " +
								 * entryNameShort + "  =  " + recFormat );
								 */
							}
							...
							if ( lobAzeptedCheck.contains( "Check" ) ) {
								...
								if ( recMsg.equals( "notAZ" ) ) {
									...
									// Dateien in diesem ordner
									if ( entryNameParent.getAbsolutePath()
											.equals( oldParentNotaz ) ) {
										if ( counterDetailDirNotaz == 3 ) {
											msgNotAz = msgNotAz + " ... [+"
													+ (counterDetailDirNotaz
															- 2)
													+ "] Temp";
										} else if ( counterDetailDirNotaz > 3 ) {
											// letzter zaehler mit Temp anpassen
											msgNotAz = msgNotAz.replace(
													(counterDetailDirNotaz - 3)
															+ "] Temp",
													(counterDetailDirNotaz - 2)
															+ "] Temp" );
										} else {
											msgNotAz = msgNotAz + ", "
													+ entryNameFile.getName();
										}
										counterDetailDirNotaz = counterDetailDirNotaz
												+ 1;
									} else {
										// neuer Ordner
									...
									oldParentNotaz = entryNameParent
											.getAbsolutePath();
								} else {
					...
					if ( !msgUnknown.equals( "" ) ) {
						...
						Logtxt.logtxt( logFile, getTextResourceService()
								.getText( locale, MESSAGE_XML_MODUL_M_SIARD )
								+ getTextResourceService().getText( locale,
										MESSAGE_XML_M_UNKNOWN, msgUnknown,
										warning, warningEl ) );
					}
					if ( !msgNotAz.equals( "" ) ) {
						// Wurde nicht erkannt (oder es gab ein Problem)
