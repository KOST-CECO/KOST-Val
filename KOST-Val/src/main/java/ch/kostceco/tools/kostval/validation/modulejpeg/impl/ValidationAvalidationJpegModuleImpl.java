...
public class ValidationAvalidationJpegModuleImpl extends ValidationModuleImpl
	...
	public boolean validate(File valDatei, File directoryOfLogfile, Map<String, String> configMap, Locale locale,
			File logFile, String dirOfJarPath) throws ValidationAjpegvalidationException {
		...
		if (null == ifmt) {
			if (min) {
				...
				Logtxt.logtxt(logFile, getTextResourceService().getText(locale, MESSAGE_XML_MODUL_A_JPEG)
						+ getTextResourceService().getText(locale, MESSAGE_XML_SERVICEINVALID, "BadPeggy", ""));
		...
		try {
				...
				for (String msg : ires.collapsedMessages()) {
					...
					if (msg.startsWith("Unsupported Image Type")) {
						...
						if (min) {
							...
							/*
							 * Fehlermeldung "Not a JPEG file: starts with ..." weiter analysieren. Meist
							 * ist zwischen zwei EOI von Thumbnails kein SOI vorhanden. Ist letzteres der
							 * Fall muss ein EOI-EOI-Fehler ausgegeben werden.
							 */
							...
							if (msg.startsWith("Corrupt JPEG data: premature end of data segment")) {
								...
								Logtxt.logtxt(logFile,
										getTextResourceService().getText(locale, MESSAGE_XML_MODUL_B_JPEG)
												+ getTextResourceService().getText(locale, ERROR_XML_A_HIT_MARKER,
														msg));
								...
								Logtxt.logtxt(logFile,
										getTextResourceService().getText(locale, MESSAGE_XML_MODUL_C_JPEG)
												+ getTextResourceService().getText(locale, ERROR_XML_B_SOS_NO_SOF,
														msg));
								...
								Logtxt.logtxt(logFile,
										getTextResourceService().getText(locale, MESSAGE_XML_MODUL_C_JPEG)
												+ getTextResourceService().getText(locale, ERROR_XML_B_SOF_NO_SOS,
														msg));
								...
								Logtxt.logtxt(logFile,
										getTextResourceService().getText(locale, MESSAGE_XML_MODUL_C_JPEG)
												+ getTextResourceService().getText(locale, ERROR_XML_B_KC_EOI_EOI,
														msg));
			...
			Logtxt.logtxt(logFile,
					getTextResourceService().getText(locale, MESSAGE_XML_MODUL_A_JPEG) + getTextResourceService()
							.getText(locale, ERROR_XML_SERVICEFAILED_EXIT, "BadPeggy", ioe.getMessage()));
			isValid = false;
			return isValid;
		}
