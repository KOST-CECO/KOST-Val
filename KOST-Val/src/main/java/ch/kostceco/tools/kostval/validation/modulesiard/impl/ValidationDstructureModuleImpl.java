				...
				if (version2) {
					...
					if (min) {
						...
						Logtxt.logtxt(logFile, getTextResourceService().getText(locale, MESSAGE_XML_MODUL_D_SIARD)
								+ getTextResourceService().getText(locale, MESSAGE_XML_D_INVALID_XMLNS, metadataXml));
					}
				}
			}
		...
		if (schemaPath.isDirectory()) {
			if (schema.getChild("tables", ns) != null) {
				...
				for (Element table : tables) {
					valid = valid && validateTable(table, ns, pathToWorkDir, schemaPath, locale, min, logFile);
					if (showOnWork) {
						if (onWork == 410) {
							onWork = 2;
							System.out.print("D-   ");
