...
public class Pngcheck {
	private static String exeDir = "resources" + File.separator + "pngcheck-3.0.3-win";
	private static String resourcesPngcheckExe = exeDir + File.separator + "pngcheck.win64.exe";

	/**
	 * fuehrt eine Validierung mit Pngcheck via cmd durch und speichert das Ergebnis
	 * in ein File (Report). Gibt zurueck ob Report existiert oder nicht
	 * 
	 * @param pngFile      png-Datei, welche validiert werden soll
	 ...
	 */
	public static String execPngcheck(File pngFile, File workDir, String dirOfJarPath) throws InterruptedException {
		boolean out = true;
		...
		if (resultExec.equals("OK")) {
			...
			resultExec = resultExec.replace("</Message><Message>ERROR: " + pngFileNormalisiert.getAbsolutePath(), "");
	...
	/**
	 * fuehrt eine Kontrolle aller benoetigten Dateien von Pngcheck durch und gibt
