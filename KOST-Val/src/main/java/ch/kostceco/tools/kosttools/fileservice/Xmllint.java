...
public class Xmllint {
	...
	/**
	 * fuehrt eine Normalisierte Validierung mit xmllint via cmd durch und gibt das
	 * Ergebnis als String zurueck
	 * 
	 * @param xmlFile      XML-Datei, welche validiert werden soll
	 * @param xsdFile      XSD-Datei, gegen welche validiert werden soll
	 ...
	 */
	public static String execXmllint(File xmlFile, File xsdFile, File workDir, String dirOfJarPath, Locale locale)
		...
		File xmlFileNormalisiert = new File(workDir + File.separator + "XML.xml");
		File xsdFileNormalisiert = new File(workDir + File.separator + xsdFile.getName());
		try {
		...
		String command = "\"\"" + exeFile.getAbsolutePath() + "\"" + " --noout --stream --nowarning --schema " + "\""
				+ xsdFileNormalisiert.getAbsolutePath() + "\"" + " " + "\"" + xmlFileNormalisiert.getAbsolutePath()
				+ "\"\"";
		 ...
		 */
		String ignor = "ERROR: " + xmlFileNormalisiert.getAbsolutePath() + " validates";
		if (resultExec.equals(ignor)) {
			 ...
			 */
			String replaceInfo = "</Message><Message>ERROR: " + xmlFileNormalisiert.getAbsolutePath()
					+ " fails to validate";
			resultExec = resultExec.replace(replaceInfo, "");
			if (locale.toString().startsWith("fr")) {
				resultExec = resultExec.replace("Schemas validity error :", "Erreur de validite des schemas :");
				resultExec = resultExec.replace("This element is not expected", "Cet element n`est pas attendu");
				resultExec = resultExec.replace("Expected is", "L`element attendu est");
				resultExec = resultExec.replace("fails to validate", "ne parvient pas a etre valide");
				resultExec = resultExec.replace("Missing child element(s).", "Elements enfants manquants.");
				resultExec = resultExec.replace("The value has a length of", "La valeur a une longueur de");
				resultExec = resultExec.replace("this underruns the allowed minimum length of",
						"ce qui est inferieur a la longueur minimale autorisee de");
				resultExec = resultExec.replace("is not a valid value of the atomic type",
						"n`est pas une valeur valide du type atomique");
			} else if (locale.toString().startsWith("de")) {
				resultExec = resultExec.replace("Schemas validity error :", "Fehler bei der Gueltigkeit des Schemas:");
				resultExec = resultExec.replace("This element is not expected", "Dieses Element wird nicht erwartet");
				resultExec = resultExec.replace("Expected is", "Erwartet wird");
				resultExec = resultExec.replace("fails to validate", "kann nicht validiert werden");
				resultExec = resultExec.replace("Missing child element(s).", "Fehlende untergeordnete Elemente.");
				resultExec = resultExec.replace("The value has a length of", "Der Wert hat eine Laenge von");
				resultExec = resultExec.replace("this underruns the allowed minimum length of",
				...
				resultExec = resultExec.replace("Schemas validity error :", "Errore nella validita dello schema:");
				resultExec = resultExec.replace("This element is not expected", "Questo elemento non e previsto");
				resultExec = resultExec.replace("Expected is", "Previsto");
				resultExec = resultExec.replace("fails to validate", "non puo essere validato");
				resultExec = resultExec.replace("Missing child element(s).", "Elementi subordinati mancanti.");
				resultExec = resultExec.replace("The value has a length of", "Il valore ha una lunghezza di");
				resultExec = resultExec.replace("this underruns the allowed minimum length of",
	...
	/**
	 * fuehrt eine Validierung mit xmllint via cmd durch und gibt das Ergebnis als
	 * String zurueck (Keine Normalisierung bei SIP)
	 * 
	 * @param xmlFile      XML-Datei, welche validiert werden soll
	 * @param xsdFile      XSD-Datei, gegen welche validiert werden soll
	 * @param workDir      Temporaeres Verzeichnis
	 ...
	 * @return String mit Validierungsergebnis ("OK" oder den Fehler.
	 */
	public static String execXmllintSip(File xmlFile, File xsdFile, File workDir, String dirOfJarPath, Locale locale)
			throws InterruptedException {
		boolean out = false;
		File exeFile = new File(dirOfJarPath + File.separator + pathToxmllintExe);
		...
		if (resultExec.equals(ignor)) {
			 ...
			 * schema0\table2\table2.xml fails to validate
			 */
			String replaceInfo = "</Message><Message>ERROR: " + xmlFile.getAbsolutePath() + " fails to validate";
			resultExec = resultExec.replace(replaceInfo, "");
			if (locale.toString().startsWith("fr")) {
				resultExec = resultExec.replace("Schemas validity error :", "Erreur de validite des schemas :");
				resultExec = resultExec.replace("This element is not expected", "Cet element n`est pas attendu");
	 ...
	 * String zurueck
	 * 
	 * @param xmlFile      XML-Datei, welche kontrolliert werden soll
	 * @param workDir      Temporaeres Verzeichnis
	 * @param dirOfJarPath String mit dem Pfad von wo das Programm gestartet wurde
	 * @return String mit Kontrollergebnis ("OK" oder den Fehler.
	 */
	public static String structXmllint(File xmlFile, File workDir, String dirOfJarPath, Locale locale)
			throws InterruptedException {
		boolean out = false;
		File exeFile = new File(dirOfJarPath + File.separator + pathToxmllintExe);
		...
		String command = "\"\"" + exeFile.getAbsolutePath() + "\"" + " --noout --stream " + "\""
				+ xmlFileNormalisiert.getAbsolutePath() + "\"\"";

		String resultStru = Cmd.execToStringSplit(command, out, workDir);
		/*
		 * C:\Program Files
		 * (x86)\KOST-CECO\KOST-Tools\KOST-Val\resources\xmllint>xmllint.exe --noout
		...
		if (resultStru.equals(ignor)) {
			 ...
			 * 2Fxsd_1a-3d-IO/content/DOS_04/shiporder1.xml:17: parser error : Opening and
			 * ending tag mismatch: titel line 0 and title</Message><Message>ERROR:
			 * <titel>Hide your heart</title></Message><Message>ERROR:
			...
			if (locale.toString().startsWith("fr")) {
				resultStru = resultStru.replace("parser error :", "erreur d'analyse syntaxique :");
				resultStru = resultStru.replace("Opening and ending tag mismatch:",
						"Mismatch des balises d'ouverture et de fin :");
				resultStru = resultStru.replace(" and ", " et ");
			} else if (locale.toString().startsWith("de")) {
				resultStru = resultStru.replace("parser error :", "Parser-Fehler:");
				resultStru = resultStru.replace("Opening and ending tag mismatch:",
	...
	/**
	 * fuehrt eine Kontrolle aller benoetigten Dateien von xmllint durch und gibt
