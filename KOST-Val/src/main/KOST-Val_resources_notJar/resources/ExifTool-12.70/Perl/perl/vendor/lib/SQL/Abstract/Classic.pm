...
sub belch (@) {
  # Fugly as hell - allow calls as functions ( backcompat ) and as methods ( subclassing )
  shift
    if @_ > 1 and length( ref ( $_[0] ) );
...
sub _assert_pass_injection_guard {
  if ($_[1] =~ $_[0]->{injection_guard}) {
    my $class = ref $_[0];
    $_[0]->puke(
       "Possible SQL injection attempt '$_[1]'. If this is indeed a part of the "
...
sub _insert_ARRAYREF { # just generate values(?,?) part (no list of fields)
  ...
  $self->{bindtype} ne 'columns'
    or $self->belch( "can't do 'columns' bindtype when called with arrayref" );
  ...
  for my $k (sort keys %$data) {
    ...
    $self->_SWITCH_refkind($v, {
      ...
      HASHREF => sub {
        ...
        $self->puke( 'Operator calls in update must be in the form { -op => $arg }' )
          if (@rest or not $op =~ /^\-(.+)/);
...
sub _where_ARRAYREF {
  ...
  $logic eq 'AND' or $logic eq 'OR' or $self->puke( "unknown logic: $logic" );
  ...
  while (@clauses) {
    ...
    my ($sql, @bind) = $self->_SWITCH_refkind($el, {
      ...
      UNDEF     => sub { $self->puke( "Supplying an empty left hand side argument is not supported in array-pairs" ) },
    });
  ...
  for my $k (sort keys %$where) {
    ...
    my ($sql, @bind) = do {
      if ($k =~ /^-./) {
        ...
        if (! length $k) {
          if ( SQL::Abstract::Util::is_literal_value( $v ) ) {
            ...
            $self->puke( "Supplying an empty left hand side argument is not supported in hash-pairs" );
          }
        }
...
sub _where_unary_op {
  ...
  if (my $op_entry = List::Util::first {$op =~ $_->{regex}} @{$self->{unary_ops}}) {
    ...
    if (not ref $handler) {
      if ($op =~ s/ [_\s]? \d+ $//x ) {
        $self->belch(
          'Use of [and|or|nest]_N modifiers is deprecated and will be removed in SQLA v2.0. '
      ...
      $self->puke( "Illegal handler for operator $op - expecting a method name or a coderef" );
    }
  }
...
sub _where_op_ANDOR {
  ...
  $self->_SWITCH_refkind($v, {
    ...
    SCALARREF  => sub {
      $self->puke(
        "-$op => \\\$scalar makes little sense, use " .
        ($op =~ /^or/i
    ...
    SCALAR => sub { # permissively interpreted as SQL
      $self->puke( "-$op => \$value makes little sense, use -bool => \$value instead" );
    },

    UNDEF => sub {
      $self->puke( "-$op => undef not supported" );
    },
   });
}
...
sub _where_op_NEST {
  ...
  $self->_SWITCH_refkind($v, {
    ...
    SCALAR => sub { # permissively interpreted as SQL
      $self->belch( "literal SQL should be -nest => \\'scalar' "
...
sub _where_op_IDENT {
  ...
  if (! defined $rhs or length ref $rhs) {
    $self->puke( "-$op requires a single plain scalar argument (a quotable identifier)" );
  }
...
sub _where_hashpair_HASHREF {
  ...
  for my $orig_op (sort keys %$v) {
    ...
    # CASE: special operators like -in or -between
      ...
      if (! $handler) {
        $self->puke( "No handler supplied for special operator $orig_op" );
      }
        ...
        $self->puke( "Illegal handler for special operator $orig_op - expecting a method name or a coderef" );
      }
    }
    else {
      $self->_SWITCH_refkind($val, {
        ...
        UNDEF => sub {          # CASE: col => {op => undef} : sql "IS (NOT)? NULL"
          ...
          : $op =~ $self->{like_op}       ? $self->belch("Supplying an undefined argument to '@{[ uc $op]}' is deprecated") && 'is'
          : $op =~ $self->{inequality_op} ? 'is not'
          : $op =~ $self->{not_like_op}   ? $self->belch("Supplying an undefined argument to '@{[ uc $op]}' is deprecated") && 'is not'
          : $self->puke( "unexpected operator '$orig_op' with undef operand" );
...
sub _where_field_op_ARRAYREF {
  ...
  if(@vals) {
    ...
    if (
      ...
      my $o = uc($op);
      $self->belch(
            "A multi-element arrayref as an argument to the inequality op '$o' "
    ...
    : $op =~ $self->{like_op}       ? $self->belch("Supplying an empty arrayref to '@{[ uc $op]}' is deprecated") && $self->{sqlfalse}
    : $op =~ $self->{inequality_op} ? $self->{sqltrue}
    : $op =~ $self->{not_like_op}   ? $self->belch("Supplying an empty arrayref to '@{[ uc $op]}' is deprecated") && $self->{sqltrue}
    : $self->puke( "operator '$op' applied on an empty array (field '$k')" );
  }
}
...
sub _where_field_BETWEEN {
  ...
  my ($clause, @bind) = $self->_SWITCH_refkind($vals, {
    ...
    ARRAYREF => sub {
      $self->puke( $invalid_args ) if @$vals != 2;
      ...
      foreach my $val (@$vals) {
        my ($sql, @bind) = $self->_SWITCH_refkind($val, {
           ...
           HASHREF => sub {
             ...
             $self->puke ("Only simple { -func => arg } functions accepted as sub-arguments to BETWEEN")
               if (@rest or $func !~ /^ \- (.+)/x);
             $self->_where_unary_op ($1 => $arg);
           },
           FALLBACK => sub {
             $self->puke( $invalid_args ),
           },
        });
        push @all_sql, $sql;
        push @all_bind, @bind;
...
sub _where_field_IN {
  ...
  my ($sql, @bind) = $self->_SWITCH_refkind($vals, {
      ...
      if (@$vals) { # nonempty list
        ...
        for my $val (@$vals) {
          my ($sql, @bind) = $self->_SWITCH_refkind($val, {
            ...
            HASHREF => sub {
              my ($func, $arg, @rest) = %$val;
              $self->puke ("Only simple { -func => arg } functions accepted as sub-arguments to IN")
                if (@rest or $func !~ /^ \- (.+)/x);
              $self->_where_unary_op ($1 => $arg);
            },
            UNDEF => sub {
              $self->puke(
                'NULL-within-IN not implemented: The upcoming SQL::Abstract::Classic 2.0 will emit the logically correct SQL instead of raising this exception.'
              );
            },
          });
          push @all_sql, $sql;
    ...
    UNDEF => sub {
      $self->puke( "Argument passed to the '$op' operator can not be undefined" );
    },

    FALLBACK => sub {
      $self->puke( "special op $op requires an arrayref (or scalarref/arrayref-ref)" );
    },
  });
  ...
  return $self->_SWITCH_refkind($arg, {
    ...
    HASHREF   => sub {
      ...
      if ( @rest or not $key =~ /^-(desc|asc)/i ) {
        $self->puke( "hash passed to _order_by must have exactly one key (-desc or -asc)" );
      }
...
sub values {
    my $self = shift;
    my $data = shift || return;
    $self->puke( "Argument to ", (ref($self) || $self), "->values must be a \\%hash" )
        unless ref $data eq 'HASH';
...
=head1 Low-impact fork of SQL::Abstract v1.81 ( 2014-10-25 )

This module is nearly identical to L<SQL::Abstract v1.81
|https://metacpan.org/release/RIBASUSHI/SQL-Abstract-1.81>. A recent
flurry of activity on the original L<SQL::Abstract> namespace risks leaving
downstream users without a way to opt out of impending developments.
Therefore this module exists to preserve the ability of users to opt into
the new way of doing things according to their own schedules.
...
when creating the C<SQL::Abstract::Classic> object.
...
a new B<SQL::Abstract::Classic> object which can then be used to generate
...
that can be applied symmetrically to fields (actually B<SQL::Abstract::Classic>
does not validate this option; it will just pass through what you specify
verbatim).
...
The problem is, B<SQL::Abstract::Classic> will normally just return a C<@bind> array,
...
Now, why would you still use B<SQL::Abstract::Classic> if you have to do this crap?
...
to extend the syntax understood by L<SQL::Abstract::Classic>.
...
The basic premise of SQL::Abstract::Classic is that in WHERE specifications the "left
...
(including references and blessed references), L<SQL::Abstract::Classic> will simply
...
are builtin within C<SQL::Abstract::Classic> (as the overridable methods
...
L<SQL::Abstract::Classic> object as:
...
internally by B<SQL::Abstract::Classic>. Thus, as long as your data retains
...
L<SQL::Abstract>, L<DBIx::Class>, L<DBIx::Abstract>, L<CGI::FormBuilder>, L<HTML::QuickTable>.
