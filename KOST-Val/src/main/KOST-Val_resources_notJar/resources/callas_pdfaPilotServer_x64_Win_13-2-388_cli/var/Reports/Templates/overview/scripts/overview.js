...
function getHitCounts(type) {
	...
	cals_res_info.steps.forEach(function(step, stepIndex) {
		if(type === "fixups" && (step.type === "fixup" || step.type === "profile")) {
			if(step.fixups.length > 0) {
...
function addOverviewIcons() {
	...
	let lastFixup = 0;
	if(cals_res_info["profile_type"] === "profile") {
		lastFixup = getHitCounts("fixups");
	}else {
		let lastStep = cals_res_info.steps[cals_res_info.steps.length-1];
		if((lastStep.type === "fixup" || lastStep.type === "profile") && lastStep.fixups.length > 0) {
			lastFixup = lastStep.fixups[lastStep.fixups.length-1].succeeded;
		}
	}
...
function addDetails() {
	...
	const isProcessPlan = cals_res_info.profile_type === "metaprofile";
	const html = `${isProcessPlan ? `<div class="process-plan-header">
		<img src="img/process.pdf"/>
		<h2>${cals_res_info.profile_name}</h2>
	</div>` : ''}
	<div class="${isProcessPlan && "process-plan-container"}">
		${cals_res_info.steps.map(function(step, index) {
...
function addStepDetails(stepIndex) {
	...
	const noProblemsTypeArray = ["profile", "rule", "fixup"]
	if(noProblemsFound && noProblemsTypeArray.includes(step.type)) {
...
document.addEventListener("DOMContentLoaded", function() {
	...
	if(typeof cals_sec_info !== 'undefined') {
		...
		cals_sec_info.hasOwnProperty("separations") && addSeparations();
		cals_sec_info.hasOwnProperty("imageresolution") && addImageResolution();
		cals_sec_info.hasOwnProperty("smallobjects") && addSmallObjects();
	}
