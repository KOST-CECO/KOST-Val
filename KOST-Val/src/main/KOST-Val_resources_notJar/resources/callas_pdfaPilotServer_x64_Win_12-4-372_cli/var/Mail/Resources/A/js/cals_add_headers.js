...
function cals_addHeadersForAppointments( parent, hsettings, display_key )
{
	var n = 0;

	if(cals_ccmip.appointment)
	{
		for(var j=0; j < displayable_appointment_infos.length; j++)
		{
			var key = displayable_appointment_infos[j];
			if( cals_ccmip.appointment[key] ) 
			{
				var display_key = cals_getString("_appointment_"+key);
				var value = cals_ccmip.appointment[key];
				if( key == "start" || key == "end" ) 
				{
					var format = cals_getString("_appointment_date_format"); // get localised format from cals_strings.js
					var rfc1123_date = value; // why rfc1123? -> http://www.csgnetwork.com/timerfc1123calc.html
					moment.lang(cals_ccmip_mapping.language); // http://momentjs.com/docs/#/i18n/changing-language/
					var offset = cals_ccmip.appointment[key+"TimezoneOffset"] ? cals_ccmip.appointment[key+"TimezoneOffset"] : 0;
					var localised_date = moment.utc(rfc1123_date).zone(offset).format(format);
					value = localised_date;
				}
				n += cals_addHeaderForKey( parent, display_key, display_key, value );
			}
		}
	}
	return n;
...
function cals_addHeaders()
{
	...
	for(var j=0; j < displayable_headers.length; j++)
	{
		...
		else if( key == "_appointments")
		{
			n += cals_addHeadersForAppointments( div, hsettings, display_key );
		}
		else
		{
